...
- `ACME_EAB_KID` - the KID value for the ACME External Account Binding. To generate ACME EAB credentials see https://zerossl.com/documentation/acme/generate-eab-credentials/
- `ACME_EAB_HMAC_KEY` - the HMAC (Hash-Based Message Authentication Code) key value for the ACME External Account Binding.
- `VAULT_TOKEN` - optional, if set use the JWT auth method with this token to authenticate against Vault.
...
You can specify the list of domains directly via the `CERTIFICATOR_DOMAINS_LIST` environment variable.  
This is useful for containerized deployments or environments where editing files is inconvenient.
...
- The value should be a comma-separated list of domains.
- **If this variable is set (non-empty), it takes precedence over file-based configuration.**

### 2. Domains File: `CERTIFICATOR_DOMAINS_FILE`

If `CERTIFICATOR_DOMAINS_LIST` is not set, the application will load domains from a YAML file specified by the `CERTIFICATOR_DOMAINS_FILE` environment variable.

- Default: `/code/domains.yml`
- An example file is in [domains.yml](domains.yml).
...
**Note:**  
The canonical list of domains used by the application is always available in the `Domains` field of the config, which is set from either the environment variable or the file, depending on the above logic.
